#!/usr/bin/env python3
"""
Enhanced parser for Taittiriya texts that handles multiple samhita files.
"""
...
class EnhancedTaittiriyaParser:
    def __init__(self, base_dir: str):
        ...
        self.checkpoint_file = self.base_dir / "enhanced_parsing_checkpoint.json"
    ...
    def parse_verses_from_paragraphs(self, paragraphs: List[str], chapter: str) -> List[Dict[str, Any]]:
        ...
        for para in paragraphs:
            ...
            if any(skip_word in para.lower() for skip_word in [
                ...
                'kRuShNa yajurvEdIya', 'prathamaH praSnaH', 'dvitIyaH praSnaH',
                'tRutIyaH praSnaH', 'caturthaH praSnaH', 'pa~jcamaH praSnaH',
                'ShaShThaH praSnaH', 'saptamaH praSnaH'
            ]):
                continue
    ...
    def get_kanda_from_chapter(self, chapter: str) -> str:
        """Extract kanda number from chapter (e.g., '2.1' -> '2')."""
        return chapter.split('.')[0]
    
    def process_samhita_file(self, kanda: str) -> Dict[str, Any]:
        """Process a samhita file for a given kanda."""
        samhita_file = self.samhita_dir / f"TS {kanda} Baraha.docx"
        ...
        verses = self.parse_verses_from_paragraphs(paragraphs, kanda)
        ...
        print(f"Extracted {len(verses)} verses from samhita kanda {kanda}")
        return result
    ...
    def process_all_files(self):
        """Process all files with enhanced multi-samhita support."""
        ...
        # Process samhita files by kanda (1-7)
        for kanda in ['1', '2', '3', '4', '5', '6', '7']:
            if kanda not in samhita_cache:
                print(f"Loading samhita for kanda {kanda}...")
                samhita_data = self.process_samhita_file(kanda)
                samhita_cache[kanda] = samhita_data
                
                # Save individual samhita file
                samhita_output = self.output_dir / f"samhita_kanda_{kanda}.json"
                ...
                print(f"Using cached samhita data for kanda {kanda}: {len(samhita_cache[kanda].get('verses', []))} verses")
        ...
        for padam_file in padam_files:
            ...
            try:
                ...
                chapter = padam_data["chapter"]
                kanda = self.get_kanda_from_chapter(chapter)
                
                # Get appropriate samhita data
                samhita_data = samhita_cache.get(kanda, {"verses": []})
    ...
    def create_final_output(self):
        ...
        final_output = {
            "metadata": {
                ...
                "match_percentage": round(total_matched / total_verses * 100, 1) if total_verses > 0 else 0,
                "parsing_method": "Enhanced multi-kanda parsing",
                "format_notes": "Each verse contains 'verse_id', 'chapter', 'padam', 'samhita', and 'has_samhita_match' fields"
            },
            "chapters": all_chapters
        }
        
        final_file = self.output_dir / "taittiriya_complete_enhanced.json"
        with open(final_file, 'w', encoding='utf-8') as f:
            json.dump(final_output, f, ensure_ascii=False, indent=2)
        
        print(f"\nFinal enhanced output created: {final_file}")
        print(f"Summary:")
        ...
        print(f"  - {final_output['metadata']['match_percentage']}% match rate")

def main():
    parser = EnhancedTaittiriyaParser("/Users/skmnktl/Projects/udapaana/data/taittiriya")
